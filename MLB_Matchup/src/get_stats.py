...
def get_team_record(team_name):
    ...
    if not PYBASEBALL_AVAILABLE:
        print("pybaseball not available - returning fallback record")
        return '69-69'
    
    try:
        current_year = datetime.now().year
        print(f"Getting record for {team_name} in {current_year}")
        # Get standings
        ...
        for division in standings:
            if team_name in division['Tm'].values:
                ...
                if not team_row.empty:
                    ...
                    print(f"Record for {team_name} in {current_year}: {int(wins)}-{int(losses)}")
