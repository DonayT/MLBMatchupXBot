...
class TwitterImageGenerator:
    def __init__(self):
        ...
        self.accent_color = (0, 100, 200)  # Blue
        ...
        try:
            ...
            self.body_font = ImageFont.truetype("arial.ttf", 18)
        except:
    ...
    def create_lineup_image(self, game_data, output_path):
        """Create a Twitter image with lineup information"""
        ...
        draw.text((title_x, 20), title, font=self.title_font, fill=self.text_color)
        
        # Draw starting pitchers
        pitcher_y = 80
        draw.text((50, pitcher_y), f"Starting Pitchers:", font=self.header_font, fill=self.accent_color)
        ...
        draw.text((50, pitcher_y + 35), away_pitcher_text, font=self.body_font, fill=self.text_color)
        draw.text((50, pitcher_y + 60), home_pitcher_text, font=self.body_font, fill=self.text_color)
        
        # Draw divider line
        draw.line([(50, pitcher_y + 90), (self.width - 50, pitcher_y + 90)], 
                  fill=self.divider_color, width=2)
        
        # Draw lineups
        lineup_y = pitcher_y + 120
        
        # Away team lineup (left side)
        draw.text((50, lineup_y), f"{game_data['away_team']} Lineup:", font=self.header_font, fill=self.accent_color)
        ...
        draw.text((home_x, lineup_y), f"{game_data['home_team']} Lineup:", font=self.header_font, fill=self.accent_color)
        ...
        draw.text((50, self.height - 40), timestamp_text, font=self.body_font, fill=self.divider_color)
